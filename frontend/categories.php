...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($_POST['action'] === 'add') {
            ...
            // Check if category has related products in school_supplies table
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM school_supplies WHERE category_id = ?");
...
// Fetch categories with total items from school_supplies
$stmt = $pdo->query("SELECT c.*, COALESCE(SUM(s.quantity), 0) as total_items 
                     FROM categories c 
...
function showDeleteConfirmModal(id, name) {
    Swal.fire({
        ...
        html: `Are you sure you want to delete category <strong>${name}</strong>?<br><span class="text-warning">Note: Categories with supplies cannot be deleted.</span>`,
        icon: 'warning',
